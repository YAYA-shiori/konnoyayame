...
.DESCRIPTION
    ...
    Findings in dic/system (the yaya-dic submodule) are hidden unless -IncludeSystem is given.
...
foreach ($line in ($result.StdOut -split "\r?\n")) {
    ...
    if ($fields.Count -ge 6 -and $fields[2] -eq 'at' -and $fields[4] -eq 'pos:') {
        ...
        $name = $fields[1]
        $file = $fields[3]
        if (-not $IncludeSystem -and $file -like 'dic/system/*') { $hiddenSystem++; continue }
...
if ($hiddenSystem -gt 0) { $summary += ", hidden in dic/system: $hiddenSystem (use -IncludeSystem)" }
