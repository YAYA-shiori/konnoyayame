...
3. 試したい内容に合わせて送る。
   ...
   - どの場合も、送った後に SSP のエラーログに増えた Warning 以上が表示される（Info と Notice は件数だけ）。
4. SSP のログをまとめて見る（読み取りのみ）:
   - エラーログ（このゴーストの分）: `tools/ssp-log.ps1`。Info や Notice も含めて全部出る。発信元を問わず見るなら `-All`
...
5. 終了コードの意味: 2 は SSP のエラーログに Error か Critical が増えた（`sstp.ps1`、`run-ssp.ps1`）、またはそれが表示された（`ssp-log.ps1`）。3 は SSP に接続できない（起動していない）。404 が返ったらゴーストが見つからない（`-Ghost <\0 の名前>` か `-AnyGhost` を指定）。
6. 吹き出しからのはみ出し、改行位置、表情はユーザーの画面にしか出ない。何を再生したかを伝えて、見た目を確認してもらう。
