...
0. 最初に `powershell -NoProfile -ExecutionPolicy Bypass -File tools/run-ssp.ps1` を実行する。`ssp.exe --ghost <このフォルダ>` を使うので、SSP にインストールしなくても作業中のフォルダがそのまま動く。作者のデスクトップにゴーストが現れるので、一言断ってから起動する。起動中に SSP のエラーログに増えた警告・エラーも表示される。
...
3. 試したい内容に合わせて送る。
   ...
   - 吹き出しの見た目を確かめる: `-Script` や `-Event` に `-Balloon` を付けると、話し終わった後の吹き出しが PNG になり、そのパスが表示される（`balloon0.png` が `\0`、`balloon1.png` が `\1`）。画像を読めるなら、それを見て次を確かめる。
     - 1 行が吹き出しの幅を超えて、意図しない位置で折り返していないか。折り返していたら、区切りのよいところに `\n` を入れる（前に置く待ちは `GHOST.md` の「トークの書き方」に合わせる）
     - 行数が多すぎて、吹き出しからはみ出したりスクロールしたりしていないか
     - `\n\n` の空行、話し手の交代が、`GHOST.md` の「トークの書き方」どおりに見えるか
     - 画像の下の `from ghost-devkit (local)` は、SSTP で送ったときに SSP が付ける表示で、トークの一部ではない
     - 撮れるのは話し終わった時点の吹き出しだけ。途中の表情の変化やテンポは写らない
...
7. 吹き出しの中の文字の並びは、手順 3 の `-Balloon` で自分で確かめてから作者に見せる。表情の移り変わり、掛け合いのテンポ、アニメーションは画像に写らないので、何を再生したかを伝えて、作者の画面で確認してもらう。画像を読めないエージェントは、吹き出しも作者に見てもらう。
