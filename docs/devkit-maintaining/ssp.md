...
- SSP の版: 各スクリプトは `tools/lib/common.ps1` の `$DevkitSspRecommendedVersion`（今は 2.8.98）以降を前提にし、版による分岐はしない。版を見るのは doctor だけで、それより古い版を recommended の不足として知らせる。版は `ssp.exe` のファイルバージョン（`2, 8, 98, 3000`）の上 3 つで比べる（`Get-DevkitSspVersion`）。新しい SSP の機能を使い始めたら、分岐を足さずに `$DevkitSspRecommendedVersion` を上げる。使っている機能は次のとおり。
